...
```
{
  ...
  referredBy: string|null,     // 추천인 uid
  joinedAt: Timestamp,         // 가입일 (신규 혜택 기간 계산용)
  newbieBonusClaimed: boolean  // 신규 첫 정답 보너스 수령 여부
}
```
...
- [ ] **쿠폰함 + 푸시알림** - 환전 신청 → Admin에서 코드 입력 → 앱 내 쿠폰함에 표시 + FCM 푸시알림. 외부 채널 불필요, Firebase FCM 무료
